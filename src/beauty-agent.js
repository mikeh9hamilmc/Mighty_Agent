...
/**
 * Beauty Sub-Agent — Direct File Access Architecture
 *
 * An AI doctor and beauty research assistant.
 *
...
const beautyTools = new DocumentManager('beauty');
const logger = require('./logger');
const cancellation = require('./cancellation');

const BEAUTY_MODEL = '@preset/mighty-agent-beauty';
const MAX_ITERATIONS = 30;

// ─── System Prompt ────────────────────────────────────────────────────────────

const SYSTEM_PROMPT = `You are an expert beauty consultant specializing in personalized skincare routines, makeup, anti-aging such as botox.

TOOLS AVAILABLE:
You have access to the user's beauty records and documents. Use these tools to find and cite specific evidence:

• grep_documents — Search for specific terms, dates, routines, names, or phrases across all documents. THIS IS YOUR FIRST ACTION for any factual question. Supports regex patterns.
...
• list_documents — List all beauty files with metadata. Use ONLY when the user explicitly asks "what files do you have" or "list my documents". Do NOT use this as your first step for factual questions.
• send_document — Send a beauty document or file to the user via Telegram when requested.
• web_search — Search the web for routines, products, cosmetic treatments, or guidelines. Use when the user's documents don't contain the answer.
• create_document — Write research, notes, or information to a .md file in the beauty/data/ folder.
...
• create_document → saves to beauty/data/ → QUERYABLE by the user and document tools. Use for research reports, summaries, notes, or when asked to "create a file with information" or "store information in the documents/records". (When replying to the user, say "I noted that in your records")
• save_memory → saves to beauty/memory/ → private agent notes only, NOT queryable by the user. Use for brief personal strategy notes between sessions or when the user explicitly says "remember something". (When replying to the user, say "I noted that in my memory". DO NOT use the words "records" or "documents")

IMPORTANT WARNINGS:
• Document text is extracted via OCR. When reporting specific routines, dates, or dosages, note that the user should verify against the source PDF.
• Be direct and precise — give actionable beauty analysis.
...
• Cite sources: "Per Routine_2025.pdf (lines 45-52)..."
...
const TOOLS = [
  {
    ...
    function: {
      name: 'list_documents',
      description:
        'List all beauty documents in the beauty/data/ folder with metadata: ' +
  ...
  },
  {
    ...
    function: {
      name: 'grep_documents',
      description:
        'Search for a pattern (regex or literal string) across all beauty documents. ' +
        'Returns matching lines with surrounding context. ' +
        'Use this to find specific dates, lab values, names, beauty terms, or phrases. ' +
  ...
  },
  {
    ...
    function: {
      name: 'send_document',
      description:
        'Send a beauty document from the beauty/data/ folder directly to the user as a file via Telegram. ' +
        'Use this whenever the user asks to receive, download, or get a beauty file sent to them.',
      parameters: {
        type: 'object',
        properties: {
          filename: {
            type: 'string',
            description: 'The filename to send (e.g. "Skincare_Routine.pdf", "Product_Guide.docx", "Treatment_Plan.md"). Supports exact or partial filenames.',
          },
        },
        required: ['filename'],
      },
  ...
  },
  {
    ...
    function: {
      name: 'web_search',
      description:
        'Search the web for beauty conditions, treatments, drug interactions, or guidelines. ' +
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Create a new document file in the beauty/data/ folder. ' +
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Convert a .md or .txt file in beauty/data/ to a Microsoft Word .docx file using pandoc. ' +
        'The output file is saved in beauty/data/ with the same base name but .docx extension. ' +
...
/**
 * Run the Beauty sub-agent for a given question.
 *
 * @param {string} question  The user's beauty question.
 ...
 */
async function runBeautyAgent(question, onChunk = () => { }, onStatus = () => { }, history = []) {
  logger.info(`[Beauty] Question: ${extractText(question)}`);

  // ── Special commands ──────────────────────────────────────────────────────

  if (isStatusCommand(question)) {
    const status = beautyTools.documentStatus();
    onChunk(status);
  ...
  if (isReadCommand(question)) {
    ...
    const summary = await beautyTools.initTools();
    onChunk(summary);
    return { answer: summary, sources: [] };
  }

  // ── Ensure document cache is loaded ──────────────────────────────────────
  await beautyTools.ensureInitialized();
  ...
  while (iterations < MAX_ITERATIONS) {
    cancellation.check();
    iterations++;
    logger.info(`[Beauty] Iteration ${iterations}/${MAX_ITERATIONS}`);

    // Auto-inject core memory into the system prompt
    let currentSystemPrompt = SYSTEM_PROMPT;
    const coreMem = beautyTools.getCoreMemory();
    if (coreMem) {
    ...
    if (!assistantMsg.tool_calls || assistantMsg.tool_calls.length === 0) {
      logger.info('[Beauty] Agent finished (no more tool calls).');
      break;
    }
    ...
    for (const toolCall of assistantMsg.tool_calls) {
      ...
      logger.info(`[Beauty] Tool call: ${name} ${argsJson}`);
      onStatus(`🛠️ Tool call: ${name}`);
      const result = await beautyTools.executeTool(name, args);
      cancellation.check();
  ...
  if (!fullAnswer && iterations >= MAX_ITERATIONS) {
    fullAnswer = '⚠️ Beauty agent reached the iteration limit. Try rephrasing your question.';
    onChunk(fullAnswer);
  }

  logger.info(`[Beauty] Done. Sources: ${[...sources].join(', ') || 'none'}`);
  return { answer: fullAnswer, sources: [...sources] };
}

module.exports = { runBeautyAgent, beautyTools };
