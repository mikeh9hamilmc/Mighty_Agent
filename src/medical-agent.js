...
• create_document → saves to medical/data/ → QUERYABLE by the user and document tools. Use for research reports, summaries, notes, or when asked to "create a file with information" or "store information in the documents/records". (When replying to the user, say "I noted that in your records")
• save_memory → saves to medical/memory/ → private agent notes only, NOT queryable by the user. Use for brief personal strategy notes between sessions or when the user explicitly says "remember something". (When replying to the user, say "I noted that in my memory". DO NOT use the words "records" or "documents")
...
async function runMedicalAgent(question, onChunk = () => { }, onStatus = () => { }, history = []) {
  logger.info(`[Medical] Question: ${extractText(question)}`);
